       ...
       GuildSettings.findOne({
           ...
           if(err){
            sendError.create({
                ...
                Guild_ID: guild.id,
                User: `guildBanAdd`,
                UserID: `guildBanAddID`,
                Error: err,
                Time: `${TheDate} || ${clock} ${amORpm}`,
                Command: `guildBanAdd Event`,
                Args: `No arguments`,
            },async (err, res) => {
                if(err) {
                    console.log(err)
               ...
               if(res.logs.guild === true){
                   guild.fetchAuditLogs().then(audit =>{
                        ...
                        let embed = new SharuruEmbed()
                            ...
                            .setAuthor({name:`Thor just used his hammer!`, iconURL:audit.entries.first().executor.displayAvatarURL()})
                            .setDescription(
                                `**❯ Member:** ${user.tag} (${user.id})
                                **❯ Banned by:** ${audit.entries.first().executor.username+"#"+audit.entries.first().executor.discriminator} (${audit.entries.first().executor.id})
                                **❯ Reason:** ${audit.entries.first().reason ?? `No reason provided!`}`
                            )
