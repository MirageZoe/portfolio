       ...
       GuildSettings.findOne({
           ...
           if(err){
            sendError.create({
                ...
                User: `channelDelete`,
                UserID: `channelDeleteID`,
                Error: err,
                Time: `${TheDate} || ${clock} ${amORpm}`,
                Command: `channelDelete Event`,
                Args: `No arguments`,
            },async (err, res) => {
                if(err) {
                    console.log(err)
            ...
            if(res.logs.channel === true){
                   channel.guild.fetchAuditLogs().then(audit =>{
                        ...
                        function slowModeCal(timeInSec) {
                            ...
                            if(timeInSec >= 61 && timeInSec <= 1800){
                        ...
                        const embed = new SharuruEmbed()
                            .setAuthor({name:`Channel Deleted!`,aiconURL:udit.entries.first().executor.displayAvatarURL()})
                            .setColor(`Random`)
                            .setDescription(
                                `**❯ Channel Name:** ${channel.name}
                                **❯ Channel ID:** ${channel.id}
                                **❯ Channel Type:** ${channel.type}
                                **❯ Channel Category:** ${channel.parentID ? `<#${channel.parentID}>` : `It wasn't under any category`}
                                **❯ Channel Topic:** ${channel.topic ?? `Didn't had any topic!`}
                                **❯ Was NSFW?:** ${channel.nsfw ? `yes` : `no`}
                                **❯ Slowmode was set to:** ${slowModeCal(channel.rateLimitPerUser)}
